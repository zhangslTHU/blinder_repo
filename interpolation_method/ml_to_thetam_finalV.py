...
# @Last Modified time: 2023-03-21 17:40

'''
convert σ-coordinate/model levels to auto-defined/modified isentropic levels (θm-coordinate)
...
test: (1) takes 0.7s; (2) takes 4.5s; (3) takes 36.0s
...
## -------------------------------------------------------------------------------
def isentropic_interpolation(levels, pressure, temperature, thetam, *args, vertical_dim=0,
    ...
    thetam : array-like (K)
        Array of modified potemperature temperature
    ...
    thetam = np.array(thetam)  # modified theta
    ...
    if np.max(thetam) < np.max(levels):
    ...
    above, below, good = find_bounding_indices(thetam, levels, vertical_dim,
    ...
    if args:
        others = metpy.interpolate.interpolate_1d(isentlevels, thetam[sorter], *(np.array(arr)[sorter] for arr in args),
